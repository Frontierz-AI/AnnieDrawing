...
| `board.toJSON(options?)`                     | Portable document, the same as `createDoc().toJSON(options?)`.              |
...
// { ok, created: string[], errors: [...], warnings: [...], moved?: string[], skipped?: [...] }
```
...
Events are `change`, `select`, `view`, `tool`, `page`, and `save`. A change describes one committed transaction. `save` reports autosave as `{ status, message?, resolve? }` with status `saving`, `saved`, `error`, or `conflict`. A conflict means the stored drawing changed elsewhere, in another tab or in a save found after local edits began; nothing is written until `resolve('load')` opens the stored drawing or `resolve('keep')` saves this one over it. An idle tab follows another tab's saves and keeps its camera and page. An unreadable stored drawing is copied to `<key>#unreadable-<time>` in the same store before new work is saved. The built-in UI shows errors and conflicts until a save succeeds. Camera, selection, and pointer drafts are local view state. They do not become drawing items.
