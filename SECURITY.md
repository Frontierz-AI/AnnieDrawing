...
Run `npm run check:licenses`, review `npm audit` and dependency changes, and preserve lockfiles. The license checker is an allowlist, not legal advice or a vulnerability scanner. Releases require a fresh review of dependency updates, sanitizer behavior and export behavior; see [the release checklist](docs/releasing.md).
