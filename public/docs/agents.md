...
The [MCP example](mcp.md) exposes the same tools without adding a network server to the library. It works over a headless document or a token-authenticated loopback bridge to a board the user explicitly connects.
