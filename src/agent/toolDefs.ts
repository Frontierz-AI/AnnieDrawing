...
const descriptions: Record<keyof typeof toolSchemas, string> = {
  ...
  board_apply:
    'Atomically apply drawing operations. Supply IDs when later operations reference new items. Relative placement avoids coordinate guessing; directional place slides when a slot is taken. Agent creates grow labeled boxes and titles to fit text (standalone text defaults to 600 wide). An omitted connector route stores elbow; elbows miss intervening boxes. Use dryRun to validate before editing. Pass expectedRevision from a prior read to reject the whole batch with STALE_REVISION if the document changed.',
  board_snapshot:
