...
const descriptions: Record<keyof typeof toolSchemas, string> = {
  ...
  board_apply:
    'Atomically apply drawing operations. Supply IDs when later operations reference new items. Relative placement avoids coordinate guessing; directional place slides when a slot is taken. A place object on the item is used when the operation omits place. Labeled nodes that would cover a similar label are placed to its right so flowchart steps do not stack. A new node that connects to an existing one must place beside that node in the same batch as the arrow. If the empty space between those boxes is more than three times the placement gap, move only the new node beside its neighbor. Do not draw a second copy of the flow off to the side, and do not tidy the whole page unless asked. Agent creates grow labeled boxes and titles to fit text (standalone text defaults to 600 wide). Omitted fills on rect, ellipse, diamond, and note store a varied palette tint. An omitted connector route stores elbow; automatic elbows prefer lanes around boxes and earlier connectors, and may pick another attachment side. Use dryRun to validate before editing. Pass expectedRevision from a prior read to reject the whole batch with STALE_REVISION if the document changed.',
  board_snapshot:
